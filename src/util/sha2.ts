 ...
 */
const HASH_512_224: Array<number> = [
  0x8c3d37c8,
  0x19544da2,
  0x73e19966,
  0x89dcd4d6,
 ...
 */
const HASH_512_256: Array<number> = [
  0x22312194,
  0xfc2bf72c,
  0x9f555fa3,
  0xc84c64c2,
 ...
 */
function Sum(pWords: Array<number>, nIndex: number): number {
  return (pWords[nIndex & 0x0f] +=
    Sigma(pWords[(nIndex + 14) & 0x0f], 17, 19, 10) +
    pWords[(nIndex + 9) & 0x0f] +
    Sigma(pWords[(nIndex + 1) & 0x0f], 7, 18, 3))
}
...
function Aggregate(nA: number, nB: number): number {
  const nC: number = (nA & 0xffff) + (nB & 0xffff)
 ...
 */
function Conglomerate(pBuffer: Array<number>, nOffset: number, ...pInputs: Array<number>): void {
  let nInput: number = 0
  let nValue: number = 0
  let nA: number = 0
  let nB: number = 0
 ...
 */
function Compress(pHash: Array<number>, pBuffer: Array<number>): void {
  const pIntermediate: Array<number> = [pHash[0], pHash[1], pHash[2], pHash[3], pHash[4], pHash[5], pHash[6], pHash[7]]
  const pWords: Array<number> = []
  let nIndex: number = 0

  while (nIndex < 16) {
    pWords[nIndex] =
      pBuffer[(nIndex << 2) + 3] |
      (pBuffer[(nIndex << 2) + 2] << 8) |
      (pBuffer[(nIndex << 2) + 1] << 16) |
      (pBuffer[nIndex << 2] << 24)
  ...
  for (nIndex = 0; nIndex < 64; nIndex++) {
    const nT0: number = pIntermediate[0]
    const nT1: number = pIntermediate[1]
    const nT2: number = pIntermediate[2]
    const nT4: number = pIntermediate[4]
    const nT: number =
      pIntermediate[7] +
      (Rotate(nT4, 6) ^ Rotate(nT4, 11) ^ Rotate(nT4, 25)) +
      ((nT4 & pIntermediate[5]) ^ (~nT4 & pIntermediate[6])) +
      ROUNDS_256[nIndex] +
      (nIndex < 16 ? pWords[nIndex] : Sum(pWords, nIndex))

    for (let nIntermediate: number = 7; nIntermediate >= 0; nIntermediate--) {
      pIntermediate[nIntermediate] =
        nIntermediate === 4
          ? Aggregate(pIntermediate[3], nT)
          : nIntermediate === 0
          ? Aggregate(
              nT,
              (Rotate(nT0, 2) ^ Rotate(nT0, 13) ^ Rotate(nT0, 22)) + ((nT0 & nT1) ^ (nT0 & nT2) ^ (nT1 & nT2)),
            )
          : pIntermediate[nIntermediate - 1]
    }
  }
...
function Hash(sData: string, bHash512: boolean, nBits: number): string {
  let sHash: string = ''
  const pHash: Array<number> = (bHash512
    ? nBits === 224
      ? HASH_512_224
      : nBits === 256
      ? HASH_512_256
  ...
  const nLength: number = typeof sData === 'string' ? sData.length : 0
  const pBuffer: Array<number> = []
  ...
  if (bHash512) {
    const pExpand: Array<number> = [0, 0, 0, 0, 0, 0, 0, 0]
    const pShift: Array<number> = [24, 16, 8, 0]
    let pIntermediate: Array<number>
    ...
    do {
      ...
      for (nB = 32; nB < 160; nB += 2) {
        const nT0 = pBuffer[nB - 30]
        const nT1 = pBuffer[nB - 29]
        const nT2 = pBuffer[nB - 4]
        const nT3 = pBuffer[nB - 3]
        ...
        Conglomerate(
          ...
          pBuffer[nB - 13],
          pBuffer[nB - 31],
          ((nT1 >>> 1) | (nT0 << 31)) ^ ((nT1 >>> 8) | (nT0 << 24)) ^ ((nT1 >>> 7) | (nT0 << 25)),
          ((nT3 >>> 19) | (nT2 << 13)) ^ ((nT2 >>> 29) | (nT3 << 3)) ^ ((nT3 >>> 6) | (nT2 << 26)),
          pBuffer[nB - 14],
          pBuffer[nB - 32],
          ((nT0 >>> 1) | (nT1 << 31)) ^ ((nT0 >>> 8) | (nT1 << 24)) ^ (nT0 >>> 7),
          ((nT2 >>> 19) | (nT3 << 13)) ^ ((nT3 >>> 29) | (nT2 << 3)) ^ (nT2 >>> 6),
        )
      }
      ...
      for (nA = 0; nA < 160; nA += 8) {
        for (nB = 0; nB < 8; nB += 2) {
          const nShift: number = nB > 0 ? 8 - nB : 0
          const pSum: Array<number> = [0, 0, 0, 0]
          ...
          Conglomerate(
            ...
            ROUNDS_512[nA + nB + 1],
            pBuffer[nA + nB + 1],
            (pIntermediate[9 + nShift] & pIntermediate[nB === 2 ? 9 : 11 + nShift]) ^
              (~pIntermediate[9 + nShift] & pIntermediate[nB === 6 ? 15 : 13 - nB]),
            ((pIntermediate[9 + nShift] >>> 14) | (pIntermediate[8 + nShift] << 18)) ^
              ((pIntermediate[9 + nShift] >>> 18) | (pIntermediate[8 + nShift] << 14)) ^
              ((pIntermediate[8 + nShift] >>> 9) | (pIntermediate[9 + nShift] << 23)),
            pIntermediate[15 - nB],
            ROUNDS_512[nA + nB],
            pBuffer[nA + nB],
            (pIntermediate[8 + nShift] & pIntermediate[nB === 2 ? 8 : 10 + nShift]) ^
              (~pIntermediate[8 + nShift] & pIntermediate[nB === 6 ? 14 : 12 - nB]),
            ((pIntermediate[8 + nShift] >>> 14) | (pIntermediate[9 + nShift] << 18)) ^
              ((pIntermediate[8 + nShift] >>> 18) | (pIntermediate[9 + nShift] << 14)) ^
              ((pIntermediate[9 + nShift] >>> 9) | (pIntermediate[8 + nShift] << 23)),
            pIntermediate[14 - nB],
          )
          Conglomerate(
            pSum,
            2,
            (pExpand[nB + 1] = pIntermediate[1 + nShift] & pIntermediate[nB === 2 ? 1 : 3 + nShift]) ^
              (pIntermediate[1 + nShift] & pIntermediate[(nB === 6 ? nB : 4 - nB) + 1]) ^
              pExpand[7 - nShift],
            ((pIntermediate[1 + nShift] >>> 28) | (pIntermediate[nShift] << 4)) ^
              ((pIntermediate[nShift] >>> 2) | (pIntermediate[1 + nShift] << 30)) ^
              ((pIntermediate[nShift] >>> 7) | (pIntermediate[1 + nShift] << 25)),
            (pExpand[nB] = pIntermediate[nShift] & pIntermediate[nB === 2 ? 0 : 2 + nShift]) ^
              (pIntermediate[nShift] & pIntermediate[nB === 6 ? nB : 4 - nB]) ^
              pExpand[6 - nShift],
            ((pIntermediate[nShift] >>> 28) | (pIntermediate[1 + nShift] << 4)) ^
              ((pIntermediate[1 + nShift] >>> 2) | (pIntermediate[nShift] << 30)) ^
              ((pIntermediate[1 + nShift] >>> 7) | (pIntermediate[nShift] << 25)),
          )
